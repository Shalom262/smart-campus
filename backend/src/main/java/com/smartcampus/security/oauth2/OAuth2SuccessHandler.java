...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(
        ...
        if (user == null) {
            response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "OAuth2 user account not found.");
            return;
        }
        if (!user.isActiveForAccess()) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN, "Account pending admin approval.");
            return;
        }
